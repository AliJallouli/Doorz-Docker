    ...
    this.form = this.fb.group<RegisterPublicForm>(
      {
        ...
        email: new FormControl('', {
          nonNullable: true,
          validators: [Validators.required, SharedValidators.email()],
        }),
  ...
  onSubmit(): void {
    ...
    this.registerService.registerPublic(payload).subscribe({
      next: (res: ApiResponse<LoginResponse>) => {
        this.logger.log('📥 Réponse de register-public:', res);
        if (res.success && res.data) {
          const msg = this.translate.instant(res.success.key);
          this.authService.handleLoginSuccess(res.data, true);
          ...
          this.logger.error('❌ Erreur d’inscription:', res.error);
      ...
      error: (err) => {
        this.logger.error('❌ Erreur réseau lors de l’inscription:', err);
        const key = resolveErrorTranslationKey(err.key || 'INTERNAL_SERVER_ERROR');
